...
splitOnUnboundedES f (Stream step ms0) =
    Stream step' (liftM (mempty, ) ms0)
  where
    -- same as 'start' helper in linesUnboundedC
    step' (t, s) = do
        res <- step s
        return $ case res of
            Stop ()
                -- NOTE: this relies on an unspecified IsSequence law:
                --     onull mempty === True
                | onull t -> Stop ()
                | otherwise -> Emit (mempty, s) t
            Skip s' -> Skip (t, s')
            Emit s' x ->
                if onull after
                   then Skip (t `mappend` x, s')
                   else Emit (Seq.drop 1 after, s') (t `mappend` before)
              where
